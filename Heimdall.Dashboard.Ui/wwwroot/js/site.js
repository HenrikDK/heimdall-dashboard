        ...
        case "cluster":
            switch (name) {
                case "node-memory-stats":
                    return `sum({__name__=~"node_memory_MemTotal_bytes|node_memory_MemFree_bytes|node_memory_Buffers_bytes|node_memory_Cached_bytes", node=~"${options.nodes}"}) by (__name__, component)`;
                ...
                case "node-cpu-stats":
                    return `sum(rate(node_cpu_seconds_total{node=~"${options.nodes}", mode=~"user|system"}[1m]))`;
                case "kube-cpu-stats":
